...
from models.loss import global_singular_smoothing_loss, local_singular_smoothing_loss, HierarchicalGofLoss
import logging
...
def parse():
    ...
    parser.add_argument('--arch', type=str, default='E:/HTC/model_weight/bert-base-uncased')
    ...
    for epoch in range(args.epoch):
        ...
        with tqdm(train) as p_bar:
            for batch in p_bar: # batch包含'input_ids''attention_mask''labels'
                ...
                label_weight_function = graph_embedding_layer.weight()[-num_class - len(depth2label) - 1: -len(depth2label)-1] # 分类器的权重向量，输入嵌入矩阵（权重共享）
                ...
                #              args.label_loss_wight * global_label_loss + \
                #              args.hie_label_loss_wight * local_label_loss + \
                #              args.gof_loss_weight * geometric_loss
                ...
                # loss_total = output['loss'] + args.gof_loss_weight * geometric_loss
                loss_total = output['loss'] + (args.gof_loss_weight * alpha) * geometric_loss#  + args.align_w * align_loss

                # if args.wandb:
                    #  wandb.log({**{f'geo/{k}': v.item() for k,v in geo_comps.items()}, 'geo/alpha': alpha})
                ...
                if update_step % args.update == 0:
                    ...
                    #     wandb.log({'epoch': epoch, 'learning_rate': current_lr, 'total_loss': loss, 'HPT_loss': masked_lm_loss.item() + multiclass_loss.item(), 'masked_lm_loss': masked_lm_loss.item(), 'multiclass_loss': multiclass_loss.item(), 'global_label_loss': args.label_loss_wight * global_label_loss, 'local_label_loss': args.hie_label_loss_wight * local_label_loss, 'geometric_loss': args.gof_loss_weight * geometric_loss.item(), 'total_loss_with_reg': loss_total.item()})
